...
def filter_by_state(id_state_date: ItemListType, state: Optional[str] = "EXECUTED") -> ItemListType:
    ...
    return [item for item in id_state_date if item.get("state") == state]


# Выход функции со статусом по умолчанию 'EXECUTED'
executed_items: ItemListType = filter_by_state(id_state_date)
print(executed_items)

# Выход функции, если вторым аргументом передано 'CANCELED'
canceled_items: ItemListType = filter_by_state(id_state_date, state="CANCELED")
...
sorted_data: ItemListType = sort_by_date(id_state_date)
print(sorted_data)
